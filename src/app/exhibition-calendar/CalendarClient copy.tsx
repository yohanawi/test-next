...
export default function Calendar() {
    ...
    const filters = ["All", "This Month", "Next Month", "Last Month", "This Year"]; // Filter options
    ...
    const filteredEvents = useMemo(() => {
        ...
        return Allcalendar.filter((event: EventItem) => {
            ...
            // Only popular events for this carousel section
    ...
    // Sort popular events: upcoming first (soonest date), then past (most recent past)
    const sortedPopularEvents = useMemo(() => {
        ...
        return [...filteredEvents].sort((a, b) => {
            ...
            if (!aUpcoming && bUpcoming) return 1;  // past after upcoming
                        ...
                        <div className="flex items-center justify-between mx-8 lg:mx-20 3xl:mx-48 md:mx-20">
                            ...
                            <div className="relative" ref={dropdownRef}>
                                <button
                                    ...
                                    className="px-4 py-2 font-sans text-xs text-gray-500 transition-all border border-gray-300 rounded-md lg:text-sm hover:text-gray-900 hover:border-gray-900"
                                >
                                    {selectedFilter} ▼
                                </button>
                                {isOpen && (
                                    <div
                                        ...
                                        className="absolute z-10 w-40 mt-2 bg-white border border-gray-300 rounded-md shadow-lg"
                                    >
                                        {filters.map((filter) => (
                                            <button
                                                key={filter}
                                                role="menuitem"
                                                onClick={() => { setSelectedFilter(filter); setIsOpen(false); }}
                                                className="block w-full px-4 py-2 text-sm text-left text-gray-600 transition hover:bg-gray-100 hover:text-gray-900"
                                            >
                                                {filter}
                                            </button>
                                        ))}
