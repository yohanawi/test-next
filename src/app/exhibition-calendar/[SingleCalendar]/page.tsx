    ...
    return (
        <>
            <section className="relative w-full lg:h-[400px] h-[300px] flex items-center text-center justify-center">
                <Image src={calendarDetail?.HeroSec?.bgImage?.data?.attributes?.url ? `${baseUrl}${calendarDetail.HeroSec.bgImage.data.attributes.url}` : ""} layout="fill" objectFit="cover" alt="Banner" />
                ...
                <div className="max-w-5xl p-4 mx-auto">
                    <div className="flex flex-col items-center justify-between gap-6 p-6 bg-white shadow-lg rounded-xl md:flex-row lg:-mt-24">
                        <div className="flex items-start w-full gap-4 md:w-2/3">
                            <Image src={calendarDetail?.CalenCrd?.logo?.data?.attributes?.url ? `${baseUrl}${calendarDetail.CalenCrd.logo.data.attributes.url}` : ""} alt="AI Journal Logo" className="object-contain w-16 h-16" width={64} height={64} />
                            <div>
                                ...
                                <h2 className="text-xl font-bold text-gray-900">{calendarDetail?.CalenCrd?.title || "AI Jundi journal 2025"}</h2>
